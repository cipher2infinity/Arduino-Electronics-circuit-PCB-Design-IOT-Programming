...
char RS485_DataBuffer[100];
char Temp_Buffer[10];


//Variables for RS485
const byte RS485_Ctrl_Pin    =   24;
...
void setup() 
{
  ...
  //Initializing UART3 for RS485 Interface
  Serial3.begin(9600);
  ...
  //Initializing RS485 controle pin
  pinMode(RS485_Ctrl_Pin, OUTPUT);        // set pin to Output
...
void loop() 
{
  ...
  SendDataOverRS485(RS485_DataBuffer, strlen(RS485_DataBuffer));
...
void Read_RTC_Parameters()
{
  ...
  strcat(RS485_DataBuffer, Temp_Buffer);          //Storing temperory buffer in RS485_DataBuffer
  strcpy(Temp_Buffer, "");                        //Clearing temperory buffer to store new analog value
  strcat(RS485_DataBuffer, "/");                  //Including / in RS485_DataBuffer

  itoa(clock.dayOfMonth, Temp_Buffer, 10);
  strcat(RS485_DataBuffer, Temp_Buffer);
  strcpy(Temp_Buffer, "");
  strcat(RS485_DataBuffer, "/");
...
void Read_Analog_Input_Channel()
{
  for(uint8_t i = 0; i < 4; i++)
  {
    ...
    //Storing temperory buffer in RS485_DataBuffer
    strcat(RS485_DataBuffer, Temp_Buffer);   

    //Clearing temperory buffer to store new analog value
    strcpy(Temp_Buffer, "");

    //Including comma in RS485_DataBuffer
    strcat(RS485_DataBuffer, ",");                                
  }
}
...
void Read_BLE_Status()
{
  ...
  if(Status == 1)
  {
    strcat(RS485_DataBuffer, "1");
  }
  else
  {
    strcat(RS485_DataBuffer, "0");
  }
 ...
 */
//Function to send data over RS232 Interface with CheckSum Byte 
void SendDataOverRS485(char *DataString, uint8_t LengthOfString)
{  
  uint8_t CheckSum_Byte=0, i;

  //Setting RS485_CTRL pin high to transmit the data
  ...
  for(i = 0; i < LengthOfString; i++)   
  {
    ...
    Serial3.write(*(DataString + i));
  }

  //Sending CheckSum Byte
  Serial.write(CheckSum_Byte);
  Serial3.write(CheckSum_Byte);
