 ...
 * 
 * Later construct a string which is seperated by comma and then store it inside 
 * SD Card.
 * 
 * Name of folder : YYYY/MM
 * Name of File : YYYY/MM/YYYY_MM_DD.csv
 */
...
/*
 * Function to create file Name for SD Card
 * Folder_Name = YYYY/MM
 * File_Name = YYYY/MM/DD_MM_YYYY.csv
 */
void Creat_File_Name()
{
  //Clearing File_Name and Folder_Name
  strcpy(File_Name, "");
  strcpy(Folder_Name, "");
  ...
  itoa(clock.year+2000, Temp_Buffer, 10);
  strcat(Folder_Name, Temp_Buffer);
  strcat(File_Name, Temp_Buffer);
  strcpy(Temp_Buffer, "");
  strcat(File_Name, "/");
  strcat(Folder_Name, "/");
  
  itoa(clock.month, Temp_Buffer, 10);
  strcat(Folder_Name, Temp_Buffer);
  strcat(File_Name, Temp_Buffer);
  strcpy(Temp_Buffer, "");
  strcat(File_Name, "/");
  
  itoa(clock.dayOfMonth, Temp_Buffer, 10);        //Converting integer value to ascii charecter and storing it in Temp_Buffer 
  strcat(File_Name, Temp_Buffer);                 //Storing temperory buffer in SD_DataBuffer
  strcpy(Temp_Buffer, "");                        //Clearing temperory buffer to store new analog value
  strcat(File_Name, "_");                         //Including / in SD_DataBuffer
...
void Storing_Data_To_SD_Card()
{
  ...
  {
    //If folder exists then open the file for writting
  ...
  {
    //If folder dose not exists then creat the folder
    //Folder_Name = YYYY/MM
    SD.mkdir(Folder_Name);
    //Serial.print(Folder_Name);
    Serial.println("Created succefully");
